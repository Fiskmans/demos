...
{
	Vec3 delta{s * (myAt.y - myAt.x), myAt.x * (p - myAt.z) - myAt.y, myAt.x * myAt.y - b * myAt.z};

	myAt += delta * StepSize;
	myPoints.push_back(myAt);
	while (myPoints.size() > myTailLength)
...
{
    ImGui::InputInt("Length", &myTailLength);
	myAt.ImGui();
...
void LorenzSystem::Plot(const char* aLabel, std::function<ImVec2(Vec3)> aReduction, ImVec2 aSize)
{
	if (ImGui::BeginChild(aLabel, aSize))
	{
		...
		if (myPoints.size() > 0)
		{
			...
			dl->AddPolyline(points.data(), points.size(), ImColor(255, 255, 255, 64), 0, 1);
		}
	}
